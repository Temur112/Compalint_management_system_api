...
class ComplaintManager:
    ...
    async def create_complaint(complaint_data, user):
        ...
        return await databse.fetch_one(complaints.select().where(complaints.c.id == id_))
    ...
    async def approve(id_):
        await databse.execute(complaints.update().where(complaints.c.id == id_).values(status=State.approved))

    @staticmethod
    async def reject(id_):
        await databse.execute(complaints.update().where(complaints.c.id == id_).values(status=State.rejected))
