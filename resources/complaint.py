...
@router.get("/getComplaints", dependencies=[Depends(oauth2_bearer)], response_model=List[ComplaintOut])
async def get_complaints(request: Request,):
    user = request.state.user
    return await ComplaintManager.get_complaints(user)

@router.post("/create", response_model=ComplaintOut, dependencies=[Depends(oauth2_bearer), Depends(is_complainer)])
...
@router.delete("/deleteComplaint/{complaint_id}", dependencies=[Depends(oauth2_bearer), Depends(is_admin)], status_code=204)
async def delete_complaint_by_id(complaint_id: int):
    await ComplaintManager.delete_complaint(complaint_id)

@router.put("/{complaint_id}/approve", dependencies=[Depends(oauth2_bearer), Depends(is_approver)], status_code=204)
async def approve_complaint(complaint_id):
    await ComplaintManager.approve(complaint_id)

@router.put("/{complaint_id}/reject", dependencies=[Depends(oauth2_bearer), Depends(is_approver)], status_code=204)
